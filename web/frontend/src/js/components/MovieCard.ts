...
export class MovieCard {
    ...
    private extraInfo: MovieExtraInfo | null = null;
    ...
    private attachEvents(): void {
        this.element.querySelector('[data-action="show-info"]')?.addEventListener('click', (event) => {
            ...
            this.toggleInfo(event as MouseEvent);
        });
        ...
        const infoDiv = this.element.querySelector(`#info-${this.config.movieId}`) as HTMLElement | null;
    ...
    private async toggleInfo(event?: MouseEvent): Promise<void> {
        ...
        // Mover el card-info-film dentro del #modal-container
    ...
    private async loadExtraInfo(): Promise<void> {
        ...
        try {
            ...
            this.extraInfo = (response.data?.[0] || null) as MovieExtraInfo | null;
    ...
    private renderInfoPanel(infoDiv: HTMLElement): void {
    ...
    private async handleUpdateFromInternet(): Promise<void> {
        try {
    ...
    private normalizeExtraInfo(movie: MovieExtraInfo | null): MovieExtraInfo {
        return {
            ...(movie || {}),
