...
export class MaintenanceView extends BaseView {
    ...
    private renderDeletedMoviesMenu(): string {
        return `
            <div class="head-result">
    ...
    private renderMissingHddMenu(label: string): string {
        return `
            <div class="head-result">
    ...
    private renderLowRatedMenu(): string {
        return `
            <div class="head-result">
    ...
    private renderOverevaluatedMenu(): string {
        return `
            <div class="head-result">
    ...
    private renderCorruptMenu(): string {
        return `
            <div class="head-result">
    ...
    private async loadMissingHdd0(): Promise<void> {
        try {
            ...
            if (countSpan) countSpan.textContent = String(movies.length);
            tbody.innerHTML = movies.map((m: any) => `
                <tr>
                    <td>${m.id_movie}</td>
    ...
    private async loadMissingHdd1(): Promise<void> {
        try {
    ...
    private async loadLowRated(): Promise<void> {
        try {
    ...
    private async loadOverevaluated(): Promise<void> {
        try {
    ...
    private async loadCorrupt(): Promise<void> {
        try {
    ...
    private async loadRepeated(): Promise<void> {
        try {
            ...
            // group by title to find duplicate pairs
            ...
            if (countSpan) countSpan.textContent = String(pairs.length);
    ...
    private async loadDeletedMovies(): Promise<void> {
        try {
            ...
            // render deleted with restore button
            ...
            tbody.querySelectorAll<HTMLButtonElement>('[data-action="restore-movie"]').forEach(button => {
    ...
    private async loadCountries(): Promise<void> {
        try {
            ...
            tbody.innerHTML = movies.map((c: any) => `
                <tr>
                    <td>${c.id_country}</td>
            ...
            tbody.querySelectorAll('.save-country').forEach(btn => {
                btn.addEventListener('click', async () => {
                    const id = Number((btn as HTMLElement).dataset.id);
                    const input = tbody.querySelector(`.country-code-input[data-id="${id}"]`) as HTMLInputElement | null;
    ...
    private async loadGenres(): Promise<void> {
        try {
            ...
            // attach delete handlers for genres
    ...
    private async openUnifiedMovieEditor(movieId: number, onSaved: () => Promise<void>): Promise<void> {
        if (!Number.isInteger(movieId) || movieId <= 0) return;
    ...
    private escapeHtml(value: any): string {
        ...
        return String(value).replace(/[&<>"']/g, (s) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' } as Record<string, string>)[s]);
    }
}
