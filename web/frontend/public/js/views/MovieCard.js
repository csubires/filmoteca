...
export class MovieCard {
    ...
    async loadExtraInfo() {
        ...
        if (this.extraInfo)
            return;
        try {
            const response = await connection.get(`/extra_info_movie/${JSON.stringify({ id_movie: this.config.movieId })}`);
            this.extraInfo = response.data?.[0];
            if (this.extraInfo) {
                infoDiv.innerHTML = `
                    <strong>${this.extraInfo.realtitle || this.config.title}</strong>
                    <img src="/covers/${this.extraInfo.id_genre}${this.extraInfo.urlpicture || ''}" loading="lazy">
