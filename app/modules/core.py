...
from .sqlite import Handler_SQL     # Manejador de la base de datos
from .models import FilmFile
from .extras import timestamp_to_date, seconds_to_time, bytes_to_human
...
from config.constant import MOVIE_PATH, MOVIEXT, DB_FILE
from config.queries import TAG_QUERY
...
class HandlerScan:
    """ Recorrer directorios, listar películas, y sacar metadatos
        Args:
            ...
            statistic (bool): Hacer entrada en estadísticas o no
        Use:
        ...
        for entrada in listObj:
            ...
            if entrada.is_dir():
                ...
                if extension in MOVIEXT and not self.isFileInBD(minPath):
                    ...
                    self.oFilmFile.file_created = timestamp_to_date(entrada.stat().st_ctime)
    ...
    def get_id_genre(self, full_path):
        ...
        if id_genre is None:
            # Para el género principal, pathfolder es solo el nombre del género
            genre_pathfolder = genre if genre else path_genre.split('/')[-1]
            id_genre = self.get_or_insert_genre(genre, False, genre_pathfolder, hash_folder)
        ...
        if subgenre is not None and id_subgenre is None:
            # Para el subgénero, pathfolder es el path completo hasta el subgénero
...
# --- METHOD QUERY DATABASE
    ...
    def get_or_insert_genre(self, genero, isubgenero, pathfolder, hash_folder):
        ...
        # Primero verificar si ya existe un género con este pathfolder
        response = self.oDTB.execute('get_id_genre_by_path', {'pathfolder': pathfolder})
        if response is not None:
            # Actualizar hash_folder y report_date si es necesario
            existing_id = response[0][0]
            self.oDTB.execute('update_date_folder', {
                'pathfolder': pathfolder,
                'hash_folder': hash_folder,
                'report_date': self.report_date
            })
            return existing_id  # Retorna el id existente

        # Si no existe por pathfolder, verificar por nombre
        response = self.oDTB.execute('get_id_genre', {'name': genero})
        if response is None:                    # Si el género no existe lo inserta
            ...
            return response[0][0]               # Retorna el id de género
